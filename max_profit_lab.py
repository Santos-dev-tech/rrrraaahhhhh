"""
Max Profit Strategy Lab — Focusing on absolute maximum P&L
Anchor: 13:00 UTC
"""
import sys, io
...
print("\n" + "=" * 105)
print("  MAX PROFIT STRATEGY LAB — 13:00 UTC")
print("=" * 105)
print(header)
print(divider)
...
combos = [
    ("BASELINE 1:3 RR", {}),
    ("1:4 RR (Highest Raw Profit from last run)", dict(rr_ratio=4.0)),
    ("1:4.5 RR", dict(rr_ratio=4.5)),
    ("1:4 RR + Risk <= 3.5", dict(rr_ratio=4.0, max_risk=3.5)),
    ("1:4 RR + Tue+Thu", dict(rr_ratio=4.0, day_filter=['Tuesday','Thursday'])),
    ("1:4 RR + LONG ONLY", dict(rr_ratio=4.0, long_only=True)),
    ("1:3.5 RR + Risk <= 3.5", dict(rr_ratio=3.5, max_risk=3.5)),
    ("1:3.5 RR + Tue+Thu", dict(rr_ratio=3.5, day_filter=['Tuesday','Thursday'])),
    ("1:3.5 RR + LONG ONLY", dict(rr_ratio=3.5, long_only=True)),
    ("1:4 RR + Tue+Thu + Risk <= 3.5", dict(rr_ratio=4.0, day_filter=['Tuesday','Thursday'], max_risk=3.5)),
    ("1:3.5 RR + Tue+Thu + Risk <= 3.5", dict(rr_ratio=3.5, day_filter=['Tuesday','Thursday'], max_risk=3.5)),
    ("1:4 RR + Tue+Thu + LONG ONLY", dict(rr_ratio=4.0, day_filter=['Tuesday','Thursday'], long_only=True)),
    ("1:3.5 RR + Tue+Thu + LONG ONLY", dict(rr_ratio=3.5, day_filter=['Tuesday','Thursday'], long_only=True)),
    ("1:3 RR + Risk <= 3.5 (Best PF from last run)", dict(max_risk=3.5)),
]
...
print(f"\n\n{'='*105}")
print("  TOP 5 SETUPS BY MAX PROFIT")
print(f"{'='*105}")
print(header); print(divider)
for s in sorted(results, key=lambda x: x['pnl'], reverse=True)[:5]:
    print_row(s)

print(f"\n\n{'='*105}")
print("  TOP 5 SETUPS BY PROFIT FACTOR")
print(f"{'='*105}")
print(header); print(divider)
for s in sorted(results, key=lambda x: x['pf'], reverse=True)[:5]:
    print_row(s)
